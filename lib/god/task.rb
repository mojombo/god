module God
  ...
  class Task
    ...
    def valid?
      ...
      if self.name.nil?
        valid = false
        God.log(self, :error, "No name was specified")
      end

      # valid_states must be specified
      if self.valid_states.nil?
        valid = false
        God.log(self, :error, "No valid_states array was specified")
      end

      # valid_states must be specified
      if self.initial_state.nil?
        valid = false
        God.log(self, :error, "No initial_state was specified")
      end
    ...
    def move(to_state)
      if !self.driver.in_driver_context?
        ...
        God.log(self, :info, "#{self.name} move '#{from_state}' to '#{to_state}'")
        ...
        God.log(self, :info, "#{self.name} moved '#{from_state}' to '#{to_state}'")
      end
    ...
    def action(a, c = nil)
      if !self.driver.in_driver_context?
        ...
        if self.respond_to?(a)
          ...
          case command
            when String
              God.log(self, :info, "#{self.name} #{a}: #{command}")

              system(command)
            when Proc
              God.log(self, :info, "#{self.name} #{a}: lambda")
    ...
    def handle_poll(condition)
      ...
      if dest
        ...
        begin
          ...
          God.log(self, :info, "#{self.name} Event registration failed, moving back to previous state")
    ...
    def log_line(watch, metric, condition, result)
      ...
      if condition.info
        Array(condition.info).each do |condition_info|
          ...
          God.log(watch, :info, messages.last)
        end
      else
      ...
      God.log(watch, :debug, debug_message)
    ...
    def notify(condition, message)
      ...
      # resolve contacts
      resolved_contacts = spec[:contacts].inject([]) do |acc, contact_name_or_group|
      ...
      unless unmatched.empty?
        God.log(condition.watch, :warn, "#{condition.watch.name} no matching contacts for '#{unmatched.join(", ")}'")
      end
      ...
      resolved_contacts.each do |c|
        ...
        begin
          ...
          God.log(condition.watch, :info, msg % [])
        rescue Exception => e
          God.log(condition.watch, :error, "#{e.message} #{e.backtrace}")
          msg = "#{condition.watch.name} Failed to deliver notification for contact: #{c.name} (#{c.base_name})"
          God.log(condition.watch, :error, msg % [])
        end
      end
    end
  end
