...
module Marshmallow
  class Connection
    ...
    def speak(room, message)
      ...
      url = URI.parse("#{base_url}/message?message_format=text&format=json&auth_token=#{@options[:token]}&from=#{@options[:from][0..14]}&notify=1&room_id=#{escaped_room}&message=#{escaped_message}")
