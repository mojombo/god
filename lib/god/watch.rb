  ...
  class Watch < Task
    ...
    def call_action(condition, action)
      ...
      before_items.each do |b|
        ...
        God.log(self, :info, "#{self.name} before_#{action}: #{info} (#{b.base_name})") if info
      end

      God.log(self, :info, "#{self.name} #{action}: #{self.send(action).to_s}") if self.send(action)
      ...
      after_items.each do |b|
        ...
        God.log(self, :info, "#{self.name} after_#{action}: #{info} (#{b.base_name})") if info
      end
    end
